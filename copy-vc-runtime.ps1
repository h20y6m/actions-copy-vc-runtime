...
$dirs = @($BinDir)
$dirs += ConvertTo-StringArray $ExtraDirs
$dirs = $dirs | Where-Object { $_ -and (Test-Path $_) } | Select-Object -Unique

$extsPattern = ConvertTo-StringArray $Exts | ForEach-Object { "*" + $_.ToLower() }

$dependentDlls = New-Object System.Collections.Generic.HashSet[string]
foreach ($dir in $dirs) {
    Write-Host "::group::Scanning $dir for dependencies"
    Get-ChildItem -Path $dir -File -Recurse -Include $extsPattern -ErrorAction SilentlyContinue | ForEach-Object {
        $file = $_.FullName
...
foreach ($depDll in $dependentDlls) {
    if ($runtimeDlls.Contains($depDll)) {
        ...
        Copy-Item $runDll.FullName -Destination $BinDir -Force
        $count += 1
    }
}
